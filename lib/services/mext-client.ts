/**
 * Japanese MEXT Food Composition Database Client
 *
 * Purpose: Typed client for MEXT staging tables
 ...
 *
 * Features:
 * - 2,478 Japanese foods with 5-digit text IDs
 * - 133 nutrients with INFOODS component codes
 * - 237,032 nutrient content values
 *
 * Data Structure:
 * - source_mext_foods: Food metadata (food_id text, name, food_group)
 * - source_mext_nutrients: Nutrient definitions (nutrient_code, name, unit)
 * - source_mext_content: Food-nutrient values (long format)
 *
 * Key design: nutrient_code is text used directly as compound_sources external_id.
 * No indirection needed — join goes straight through nutrient_code (same as NEVO/FOODfiles).
 *
...
/**
 * MEXT staging table food result
 */
export interface MextStagingFood {
  foodId: string;
  name: string;
  nameEn: string | null;
...
/**
 * MEXT staging table nutrient result (with Nutri compound mapping)
 */
export interface MextStagingNutrient {
  nutrientCode: string;
  name: string;
  unit: string;
  sourceUnit: string;
 ...
 */
class MextStagingClient {
  /**
   * Search foods by name in staging table
   */
  async searchFoods(query: string, limit: number = 20): Promise<MextStagingFood[]> {
    ...
    try {
      const results = await db.execute<{
        ...
        sql`SELECT food_id, name, name_en, food_group FROM source_mext_foods WHERE (`
      ...
      logger.debug(
        {
          service: 'mext-staging',
          query,
          count: foods.length,
          durationMs: Date.now() - startTime,
        },
        'MEXT staging search completed'
      );
   ...
   */
  async getFood(foodId: string): Promise<MextStagingFood | null> {
    const results = await db.execute<{
      food_id: string;
      name: string;
      name_en: string | null;
   ...
   * (no indirection — same pattern as NEVO/FOODfiles)
   *
   * @param foodId - MEXT food ID (text, e.g., "01001")
   */
  async getNutrients(foodId: string): Promise<MextStagingNutrient[]> {
    const startTime = Date.now();
    ...
    try {
      const results = await db.execute<{
        ...
        JOIN source_mext_nutrients n ON n.nutrient_code = c.nutrient_code
        LEFT JOIN compound_sources cs ON cs.external_id = c.nutrient_code
          AND cs.external_source = 'MEXT'
      ...
      logger.debug(
        {
          ...
          mappedCount: nutrients.filter((n: MextStagingNutrient) => n.compoundId).length,
          durationMs: Date.now() - startTime,
        },
        'MEXT staging nutrients fetched'
      );
      ...
      logger.error(
        ...
        'MEXT staging nutrient fetch failed'
      );
      throw error;
    }
  }
...
/**
 * Singleton MEXT staging client instance
 */
export const mextStagingClient = new MextStagingClient();
