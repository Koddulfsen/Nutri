/**
 * Dutch NEVO Food Composition Database Search API Endpoint
 *
 * GET /api/foods/nevo/search
 *
 * Purpose: Search NEVO foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns NEVO food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal Dutch NEVO search input
 *
 * Data source: source_nevo_foods table (2,328 Dutch foods)
...
/**
 * NEVO search result (unified format)
 */
interface NevoSearchResult {
  apiSource: 'NEVO';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * NEVO search response
 */
interface NevoSearchResponse {
  results: NevoSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/nevo/search
 * Search NEVO staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<NevoSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing NEVO search'
    );

    // Step 2: Search NEVO foods with nutrient count
    ...
    const nevoResults: NevoSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'NEVO' as const,
      apiId: String(food.food_id),
      name: food.name,
      description: food.name_nl || undefined,
    ...
    logger.info(
      {
        service: 'nevo-search-api',
        query: params.q,
        count: nevoResults.length,
        durationMs,
      },
      'NEVO search completed'
    );
