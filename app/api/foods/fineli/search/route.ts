/**
 * Finnish Fineli Food Composition Database Search API Endpoint
 *
 * GET /api/foods/fineli/search
 *
 * Purpose: Search Fineli foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns Fineli food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal Finnish Fineli search input
 *
 * Data source: source_fineli_foods table (4,238 Finnish foods)
...
/**
 * Fineli search result (unified format)
 */
interface FineliSearchResult {
  apiSource: 'FINELI';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * Fineli search response
 */
interface FineliSearchResponse {
  results: FineliSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/fineli/search
 * Search Fineli staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<FineliSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing Fineli search'
    );

    // Step 2: Search Fineli foods with nutrient count
    ...
    const fineliResults: FineliSearchResult[] = paginatedResults.map((food: any) => ({
    ...
    logger.info(
      {
        service: 'fineli-search-api',
        query: params.q,
        count: fineliResults.length,
        durationMs,
      },
      'Fineli search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: fineliResults,
      metadata: {
        query: params.q,
        durationMs,
        count: fineliResults.length,
