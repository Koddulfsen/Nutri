/**
 * French CIQUAL Food Composition Database Search API Endpoint
 *
 * GET /api/foods/ciqual/search
 *
 * Purpose: Search CIQUAL foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns CIQUAL food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal French CIQUAL search input
 *
 * Data source: source_ciqual_foods table (3,484 French foods)
...
/**
 * CIQUAL search result (unified format)
 */
interface CiqualSearchResult {
  apiSource: 'CIQUAL';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * CIQUAL search response
 */
interface CiqualSearchResponse {
  results: CiqualSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/ciqual/search
 * Search CIQUAL staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<CiqualSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing CIQUAL search'
    );

    // Step 2: Search CIQUAL foods with nutrient count
    ...
    const ciqualResults: CiqualSearchResult[] = paginatedResults.map((food: any) => ({
    ...
    logger.info(
      ...
      'CIQUAL search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: ciqualResults,
      metadata: {
        query: params.q,
        durationMs,
        count: ciqualResults.length,
