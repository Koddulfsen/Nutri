/**
 * UK CoFID (Composition of Foods Integrated Dataset) Search API Endpoint
 *
 * GET /api/foods/uk-cofid/search
 *
 * Purpose: Search CoFID foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns CoFID food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal UK CoFID search input
 *
 * Data source: source_cofid_foods table (2,886 UK foods)
...
/**
 * CoFID search result (unified format)
 */
interface CofidSearchResult {
  apiSource: 'UK_COFID';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * CoFID search response
 */
interface CofidSearchResponse {
  results: CofidSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/uk-cofid/search
 * Search CoFID staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<CofidSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing CoFID search'
    );

    // Step 2: Search CoFID foods with nutrient count
    ...
    const results = await db.execute<{
      ...
      FROM source_cofid_foods f
      LEFT JOIN (
        SELECT food_code, COUNT(*) as nutrient_count
        FROM source_cofid_content
        WHERE value IS NOT NULL AND value > 0
        GROUP BY food_code
      ) cnt ON cnt.food_code = f.food_code
    ...
    const cofidResults: CofidSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'UK_COFID' as const,
      apiId: food.food_code,
      name: food.name,
      description: food.description || undefined,
      category: food.food_group ? `Group: ${food.food_group}` : undefined,
    ...
    logger.info(
      {
        service: 'cofid-search-api',
        query: params.q,
        count: cofidResults.length,
        durationMs,
      },
      'CoFID search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: cofidResults,
      metadata: {
        query: params.q,
        durationMs,
        count: cofidResults.length,
