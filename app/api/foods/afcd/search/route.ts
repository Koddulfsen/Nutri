/**
 * AFCD (Australian Food Composition Database) Search API Endpoint
 *
 * GET /api/foods/afcd/search
 *
 * Purpose: Search AFCD foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns AFCD food results with nutrient counts
 *   - Used by Add Food modal AFCD search input
 *
 * Data source: source_afcd_foods table (1,588 Australian foods)
...
/**
 * AFCD search result (unified format)
 */
interface AfcdSearchResult {
  apiSource: 'AFCD';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * AFCD search response
 */
interface AfcdSearchResponse {
  results: AfcdSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/afcd/search
 * Search AFCD staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<AfcdSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing AFCD search'
    );

    // Step 2: Search AFCD foods with nutrient count
    ...
    const results = await db.execute<{
      ...
      FROM source_afcd_foods f
      LEFT JOIN (
        SELECT afcd_food_key, COUNT(*) as nutrient_count
        FROM source_afcd_content
        GROUP BY afcd_food_key
      ) cnt ON cnt.afcd_food_key = f.afcd_food_key
    ...
    // Algorithm: Text relevance (0-1400) + nutrient count boost (0-200)
    const DELIMITERS = /[,\s\-\(\)\[\]:;\/]+/;
    ...
    const afcdResults: AfcdSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'AFCD' as const,
      apiId: food.afcd_food_key,
      name: food.name,
      description: food.derivation || undefined,
      category: food.classification ? `Classification: ${food.classification}` : undefined,
    ...
    logger.info(
      {
        service: 'afcd-search-api',
        query: params.q,
        count: afcdResults.length,
        durationMs,
      },
      'AFCD search completed'
    );
