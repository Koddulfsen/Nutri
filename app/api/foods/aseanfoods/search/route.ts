/**
 * ASEAN Food Composition Database Search API Endpoint
 *
 * GET /api/foods/aseanfoods/search
 *
 * Purpose: Search ASEANFOODS foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns ASEANFOODS food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal ASEAN search input
 *
 * Data source: source_aseanfoods_foods table (~517 Southeast Asian foods)
...
/**
 * ASEANFOODS search result (unified format)
 */
interface AseanfoodsSearchResult {
...
/**
 * ASEANFOODS search response
 */
interface AseanfoodsSearchResponse {
  results: AseanfoodsSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/aseanfoods/search
 * Search ASEANFOODS staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<AseanfoodsSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing ASEANFOODS search'
    );

    // Step 2: Search ASEANFOODS foods with nutrient count
    ...
    const results = await db.execute<{
      ...
      FROM source_aseanfoods_foods f
      LEFT JOIN (
    ...
    const aseanfoodsResults: AseanfoodsSearchResult[] = paginatedResults.map((food: any) => ({
    ...
    logger.info(
      {
        service: 'aseanfoods-search-api',
        query: params.q,
        count: aseanfoodsResults.length,
        durationMs,
      },
      'ASEANFOODS search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: aseanfoodsResults,
      metadata: {
        query: params.q,
        durationMs,
        count: aseanfoodsResults.length,
    ...
    logger.error(
      ...
      'ASEANFOODS search error'
    );

    return NextResponse.json(
      {
        error: 'ASEANFOODS search failed',
