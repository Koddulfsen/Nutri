/**
 * German BLS Food Composition Database Search API Endpoint
 *
 * GET /api/foods/bls/search
 *
 * Purpose: Search BLS foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns BLS food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal German BLS search input
 *
 * Data source: source_bls_foods table (7,140 German foods)
...
/**
 * BLS search result (unified format)
 */
interface BlsSearchResult {
  apiSource: 'BLS';
  apiId: string;
  name: string;
  description?: string;
 ...
 * BLS search response
 */
interface BlsSearchResponse {
  results: BlsSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/bls/search
 * Search BLS staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<BlsSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing BLS search'
    );

    // Step 2: Search BLS foods with nutrient count
    ...
    const blsResults: BlsSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'BLS' as const,
      apiId: food.food_code,
      name: food.name,
      description: food.name_de || undefined,
    ...
    logger.info(
      {
        service: 'bls-search-api',
        query: params.q,
        count: blsResults.length,
        durationMs,
      },
      'BLS search completed'
    );
