/**
 * New Zealand FOODfiles Food Composition Database Search API Endpoint
 *
 * GET /api/foods/foodfiles/search
 *
 * Purpose: Search FOODfiles foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns FOODfiles food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal New Zealand FOODfiles search input
 *
 * Data source: source_foodfiles_foods table (2,857 NZ foods)
...
/**
 * FOODfiles search result (unified format)
 */
interface FoodfilesSearchResult {
  apiSource: 'FOODFILES';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * FOODfiles search response
 */
interface FoodfilesSearchResponse {
  results: FoodfilesSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/foodfiles/search
 * Search FOODfiles staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<FoodfilesSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing FOODfiles search'
    );

    // Step 2: Search FOODfiles foods with nutrient count
    ...
    const foodfilesResults: FoodfilesSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'FOODFILES' as const,
      apiId: food.food_id,
      name: food.name,
      description: food.short_name || undefined,
    ...
    logger.info(
      {
        service: 'foodfiles-search-api',
        query: params.q,
        count: foodfilesResults.length,
        durationMs,
      },
      'FOODfiles search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: foodfilesResults,
      metadata: {
        query: params.q,
        durationMs,
        count: foodfilesResults.length,
    ...
    logger.error(
      ...
      'FOODfiles search error'
    );

    return NextResponse.json(
      {
        error: 'FOODfiles search failed',
