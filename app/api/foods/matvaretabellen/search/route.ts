/**
 * Norwegian Matvaretabellen Food Composition Database Search API Endpoint
 *
 * GET /api/foods/matvaretabellen/search
 *
 * Purpose: Search Matvaretabellen foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns Matvaretabellen food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal Norwegian Matvaretabellen search input
 *
 * Data source: source_matvaretabellen_foods table (2,121 Norwegian foods)
...
/**
 * Matvaretabellen search result (unified format)
 */
interface MatvaretabellenSearchResult {
...
/**
 * Matvaretabellen search response
 */
interface MatvaretabellenSearchResponse {
  results: MatvaretabellenSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/matvaretabellen/search
 * Search Matvaretabellen staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<MatvaretabellenSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    if (!validationResult.success) {
      logger.warn(
        {
          service: 'matvaretabellen-search-api',
    ...
    logger.info(
      ...
      'Executing Matvaretabellen search'
    );

    // Step 2: Search Matvaretabellen foods with nutrient count
    ...
    const results = await db.execute<{
      ...
      FROM source_matvaretabellen_foods f
      LEFT JOIN (
    ...
    const matvaretabellenResults: MatvaretabellenSearchResult[] = paginatedResults.map((food: any) => ({
      apiSource: 'MATVARETABELLEN' as const,
      apiId: food.food_id,
      name: food.name,
      category: food.food_group_id || undefined,
    ...
    logger.info(
      {
        service: 'matvaretabellen-search-api',
        query: params.q,
        count: matvaretabellenResults.length,
        durationMs,
      },
      'Matvaretabellen search completed'
    );

    // Step 5: Return results
    return NextResponse.json({
      results: matvaretabellenResults,
      metadata: {
        query: params.q,
        durationMs,
        count: matvaretabellenResults.length,
    ...
    logger.error(
      ...
      'Matvaretabellen search error'
    );

    return NextResponse.json(
      {
        error: 'Matvaretabellen search failed',
