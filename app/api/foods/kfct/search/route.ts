/**
 * Korean KFCT Food Composition Database Search API Endpoint
 *
 * GET /api/foods/kfct/search
 *
 * Purpose: Search KFCT foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns KFCT food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Searches both Korean and English food names
 *   - Used by Add Food modal Korean KFCT search input
 *
 * Data source: source_kfct_foods table (2,733 Korean foods)
...
/**
 * KFCT search result (unified format)
 */
interface KfctSearchResult {
  apiSource: 'KFCT';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * KFCT search response
 */
interface KfctSearchResponse {
  results: KfctSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/kfct/search
 * Search KFCT staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<KfctSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing KFCT search'
    );

    // Step 2: Search KFCT foods with nutrient count
    ...
    const kfctResults: KfctSearchResult[] = paginatedResults.map((food: any) => ({
    ...
    logger.info(
      {
        service: 'kfct-search-api',
        query: params.q,
        count: kfctResults.length,
        durationMs,
      },
      'KFCT search completed'
    );
