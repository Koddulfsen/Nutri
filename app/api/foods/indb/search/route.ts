/**
 * Indian INDB Nutrient Database Search API Endpoint
 *
 * GET /api/foods/indb/search
 *
 * Purpose: Search INDB foods from staging table
 * Pattern: PostgreSQL search with ILIKE matching
 * Features:
 *   - Query parameter: q (search query)
 *   - Pagination: page, limit (default 20, max 50)
 *   - Returns INDB food results with nutrient counts
 *   - Token-based scoring with nutrient count boost
 *   - Used by Add Food modal Indian INDB search input
 *
 * Data source: source_indb_foods table (1,014 Indian foods)
...
/**
 * INDB search result (unified format)
 */
interface IndbSearchResult {
  apiSource: 'INDB';
  apiId: string;
  name: string;
  description?: string;
...
/**
 * INDB search response
 */
interface IndbSearchResponse {
  results: IndbSearchResult[];
  metadata: {
    query: string;
    durationMs: number;
    count: number;
...
/**
 * GET /api/foods/indb/search
 * Search INDB staging table
 */
export const maxDuration = 60;

export async function GET(
  request: NextRequest
): Promise<NextResponse<IndbSearchResponse | { error: string; details?: unknown }>> {
  ...
  try {
    ...
    logger.info(
      ...
      'Executing INDB search'
    );

    // Step 2: Search INDB foods with nutrient count
    ...
    logger.info(
      {
        service: 'indb-search-api',
        query: params.q,
        count: indbResults.length,
        durationMs,
      },
      'INDB search completed'
    );
