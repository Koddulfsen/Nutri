  ...
  try {
    const interactionSeeds: InteractionSeedData[] = [
...

      {
        ...
        severity: 'HIGH',
        evidenceLevel: 'TIER_1_RCT',
        interactionDescription: 'Grapefruit compounds (naringin, bergamottin) inhibit CYP3A4 intestinal enzymes responsible for metabolizing statins. This inhibition can increase statin blood levels by 300-500%, dramatically raising the risk of muscle toxicity (rhabdomyolysis), liver damage, and kidney failure.',
    ...
    for (const seed of interactionSeeds) {
      try {
        ...
        console.warn(`    ├─ WARNING: Skipping interaction - compound not found: "${seed.compoundName}"`);
        // Skip this interaction instead of failing
        continue;
      }
    }
