/**
 * Main Seed Script - HTTP Version
 *
 * Uses Supabase HTTP client instead of direct PostgreSQL connection.
 * This bypasses IPv6 network connectivity issues.
 *
...
import { seedCompounds } from './compounds-http';
import { seedCategories } from './categories-http';
import { seedUsers } from './users-http';
import seedResearchCitations from './research-http';
import { seedInteractions } from './interactions-http';
import { seedValidationRanges } from './validation-http';

async function seedDatabase() {
  console.log('\n🌱 Starting Nutri Database Seeding (HTTP Mode)...\n');
  ...
  try {
    ...
    console.log('\n' + '═'.repeat(60));
    console.log('🎉 DATABASE SEEDING COMPLETE (HTTP MODE)!');
    console.log('═'.repeat(60));
